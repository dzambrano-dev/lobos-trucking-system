...
before(async () => {
  testEnvironment = await initializeTestEnvironment({ projectId, firestore: {host: '127.0.0.1', port: 8085, rules: fs.readFileSync('firestore.rules','utf8')} });
});
...
test('drivers cannot access billing, change roles, or activate staff access', async () => {
  const db = testEnvironment.authenticatedContext(driverId).firestore();
  for (const name of ['jobs', 'invoices', 'payments', 'payment_reversals', 'expenses', 'settings']) {
    await assertFails(getDoc(doc(db, name, 'private')));
    await assertFails(setDoc(doc(db, name, 'private'), {amount: 10}));
  }
